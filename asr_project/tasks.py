...
def download_youtube_video(info):
    ...
    command = f"yt-dlp -o '{info['dir_path']}/%(id)s.%(ext)s' -f 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]' {info['youtube_link']}"
    print(command)
    process = subprocess.Popen(command, shell=True)
    process.wait()
...
def perform_speech_recognition(info):
    ...
    model = 'medium'
    command = f"whisper --model {model} --output_dir {info['dir_path']} --output_format srt --initial_prompt '{info['initial_prompt']}' --word_timestamps True --max_line_width 40 {info['video_file_path']}"
    print(command)
    process = subprocess.Popen(command, shell=True)
    process.wait()
